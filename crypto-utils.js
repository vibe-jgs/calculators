...
async function encryptState(stateObj) {
  try {
    if (!window.crypto || !window.crypto.subtle) {
      console.warn("Web Crypto API unavailable, falling back to base64 encoding (insecure context).");
      return { iv: 'none', data: btoa(encodeURIComponent(JSON.stringify(stateObj))) };
    }
...
async function decryptState(encryptedObj) {
  ...
  try {
    if (encryptedObj.iv === 'none') {
      return JSON.parse(decodeURIComponent(atob(encryptedObj.data)));
    }
    const key = await getOrGenerateKey();
